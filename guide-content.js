...

{
  ...
  title:{
    fr:'Votre abonnement, en un coup d\'œil',
    en:'Your subscription, at a glance',
    el:'Η συνδρομή σας, με μια ματιά',
    de:'Ihr Abonnement auf einen Blick',
    es:'Su suscripción, de un vistazo'
  },
  ...
  body:{
    fr:"<p>Toujours dans <strong>Réglages</strong>, la section <strong>Mon abonnement</strong> rappelle votre formule active (Menu QR ou Commandes & Services) et son tarif — les deux forfaits sont facturés à l'année, il n'existe pas d'option mensuelle.</p><p>Vous y trouvez aussi un lien à partager pour recommander GeNext, et un bouton <strong>Nous contacter</strong> pour toute question.</p>",
    en:'<p>Still in <strong>Settings</strong>, the <strong>My subscription</strong> section reminds you of your active plan (Menu QR or Orders & Services) and its price — both plans are billed yearly, there is no monthly option.</p><p>You\'ll also find a shareable link to recommend GeNext, and a <strong>Contact us</strong> button for any question.</p>',
    el:'<p>Πάντα στις <strong>Ρυθμίσεις</strong>, η ενότητα <strong>Η συνδρομή μου</strong> σας υπενθυμίζει το ενεργό πακέτο σας (Menu QR ή Παραγγελίες & Υπηρεσίες) και την τιμή του — και τα δύο πακέτα χρεώνονται ετησίως, δεν υπάρχει μηνιαία επιλογή.</p><p>Εκεί θα βρείτε επίσης έναν σύνδεσμο για να προτείνετε το GeNext, και ένα κουμπί <strong>Επικοινωνήστε μαζί μας</strong> για κάθε ερώτηση.</p>',
    de:'<p>Ebenfalls unter <strong>Einstellungen</strong> zeigt der Bereich <strong>Mein Abonnement</strong> Ihr aktives Paket (Menu QR oder Bestellungen & Service) und dessen Preis — beide Pakete werden jährlich abgerechnet, es gibt keine monatliche Option.</p><p>Dort finden Sie auch einen teilbaren Link, um GeNext zu empfehlen, und eine Schaltfläche <strong>Kontaktieren Sie uns</strong> für jede Frage.</p>',
    es:'<p>Siempre en <strong>Ajustes</strong>, la sección <strong>Mi suscripción</strong> le recuerda su plan activo (Menu QR o Pedidos y Servicios) y su tarifa — ambos planes se facturan anualmente, no existe opción mensual.</p><p>También encontrará un enlace para compartir y recomendar GeNext, y un botón <strong>Contáctenos</strong> para cualquier pregunta.</p>'
  },
  kw:{fr:['abonnement','forfait','tarif','recommander','contact'],en:['subscription','plan','price','recommend','contact'],el:['συνδρομή','πακέτο','τιμή'],de:['abonnement','paket','preis'],es:['suscripción','plan','tarifa']}
},

{
  id:'adm-paiement', cat:'admin', img:'09-admin-abonnement.webp',
  title:{
    fr:'Comment se passe le paiement',
    en:'How payment works',
    el:'Πώς γίνεται η πληρωμή',
    de:'So funktioniert die Zahlung',
    es:'Cómo funciona el pago'
  },
  imgAlt:{fr:'Bouton de paiement',en:'Payment button',el:'Κουμπί πληρωμής',de:'Zahlungsschaltfläche',es:'Botón de pago'},
  body:{
    fr:"<p><strong>Aucun prélèvement automatique n'est jamais effectué</strong> sur votre compte : vous gardez la main sur chaque paiement. À l'approche d'une échéance, un bouton <strong>💳 Payer</strong> apparaît dans <strong>Mon abonnement</strong> et vous propose deux façons de régler, à votre convenance :</p><ul><li><strong>Par virement bancaire</strong> — nos coordonnées et notre IBAN s'affichent, avec un bouton pour les copier en un geste. C'est vous qui effectuez le virement vous-même, une fois par an, quand cela vous arrange.</li><li><strong>En espèces</strong> — indiquez-nous simplement votre adresse et la date qui vous convient, en un message déjà prêt sur WhatsApp, Viber ou par email. Nous envoyons alors notre commercial directement chez vous pour récupérer le règlement, sans que vous ayez à vous déplacer <em>(en Grèce, ce service est actuellement proposé à Athènes)</em>.</li></ul><p>Vous souhaitez continuer avec nous ? Il vous suffit de régler votre échéance normalement, comme ci-dessus. Vous préférez marquer une pause ? Ne faites rien : votre menu se met simplement en veille, vos données restent intactes en coulisses, et vous pouvez revenir dès que vous le souhaitez, en un clic. <strong>0 engagement — c'est toujours vous qui décidez.</strong></p>",
    en:'<p><strong>No automatic debit is ever taken</strong> from your account: you stay in control of every payment. As a due date approaches, a <strong>💳 Pay</strong> button appears in <strong>My subscription</strong> and offers two ways to pay, whichever suits you best:</p><ul><li><strong>By bank transfer</strong> — our details and IBAN are shown, with a button to copy them in one tap. You make the transfer yourself, once a year, whenever it suits you.</li><li><strong>In cash</strong> — simply let us know your address and a date that works for you, in a message that\'s already prepared on WhatsApp, Viber or email. We then send our representative straight to you to collect the payment in person, so you don\'t have to travel anywhere <em>(in Greece, this service is currently available in Athens)</em>.</li></ul><p>Want to continue with us? Simply settle your due date as above. Prefer to take a break? Do nothing: your menu quietly goes to sleep, your data stays safe and untouched behind the scenes, and you can come back whenever you like, in one click. <strong>0 commitment — you\'re always the one who decides.</strong></p>',
    el:'<p><strong>Καμία αυτόματη χρέωση δεν πραγματοποιείται ποτέ</strong> στον λογαριασμό σας: κρατάτε πάντα τον έλεγχο κάθε πληρωμής. Καθώς πλησιάζει μια λήξη, εμφανίζεται ένα κουμπί <strong>💳 Πληρωμή</strong> στην ενότητα <strong>Η συνδρομή μου</strong> που σας προσφέρει δύο τρόπους πληρωμής, όποιον σας βολεύει:</p><ul><li><strong>Με τραπεζικό έμβασμα</strong> — εμφανίζονται τα στοιχεία μας και το IBAN, με κουμπί για αντιγραφή με ένα άγγιγμα. Εσείς πραγματοποιείτε το έμβασμα, μία φορά τον χρόνο, όποτε σας βολεύει.</li><li><strong>Με μετρητά</strong> — απλώς πείτε μας τη διεύθυνσή σας και μια ημερομηνία που σας βολεύει, με ένα μήνυμα ήδη έτοιμο στο WhatsApp, στο Viber ή στο email. Στέλνουμε τότε τον εκπρόσωπό μας απευθείας σε εσάς για να παραλάβει την πληρωμή, χωρίς να χρειάζεται να μετακινηθείτε <em>(στην Ελλάδα, αυτή η υπηρεσία προσφέρεται προς το παρόν στην Αθήνα)</em>.</li></ul><p>Θέλετε να συνεχίσετε μαζί μας; Απλώς τακτοποιήστε τη λήξη σας όπως παραπάνω. Προτιμάτε μια παύση; Μην κάνετε τίποτα: το μενού σας μπαίνει απλά σε αδράνεια, τα δεδομένα σας παραμένουν ανέπαφα στο παρασκήνιο, και μπορείτε να επιστρέψετε όποτε θέλετε, με ένα κλικ. <strong>0 δέσμευση — πάντα εσείς αποφασίζετε.</strong></p>',
    de:'<p><strong>Es wird niemals automatisch Geld von Ihrem Konto abgebucht</strong>: Sie behalten bei jeder Zahlung die volle Kontrolle. Kurz vor einem Fälligkeitsdatum erscheint im Bereich <strong>Mein Abonnement</strong> eine Schaltfläche <strong>💳 Bezahlen</strong> mit zwei Zahlungswegen, ganz wie es Ihnen passt:</p><ul><li><strong>Per Banküberweisung</strong> — unsere Daten und IBAN werden angezeigt, mit einer Schaltfläche zum Kopieren mit einem Fingertipp. Sie überweisen selbst, einmal im Jahr, wann es Ihnen passt.</li><li><strong>Bar</strong> — teilen Sie uns einfach Ihre Adresse und einen passenden Termin mit, über eine bereits vorbereitete Nachricht per WhatsApp, Viber oder E-Mail. Wir schicken Ihnen dann unseren Vertreter direkt vorbei, um die Zahlung persönlich entgegenzunehmen — Sie müssen nirgendwohin fahren <em>(in Griechenland wird dieser Service derzeit in Athen angeboten)</em>.</li></ul><p>Möchten Sie mit uns weitermachen? Begleichen Sie einfach Ihre Fälligkeit wie oben beschrieben. Lieber eine Pause einlegen? Tun Sie einfach nichts: Ihr Menü legt sich still schlafen, Ihre Daten bleiben im Hintergrund unangetastet, und Sie können jederzeit mit einem Klick zurückkehren. <strong>0 Verpflichtung — Sie entscheiden immer selbst.</strong></p>',
    es:'<p><strong>Nunca se realiza ningún cargo automático</strong> en su cuenta: usted mantiene el control de cada pago. Al acercarse una fecha de vencimiento, aparece un botón <strong>💳 Pagar</strong> en <strong>Mi suscripción</strong> que le ofrece dos formas de pagar, la que más le convenga:</p><ul><li><strong>Por transferencia bancaria</strong> — se muestran nuestros datos e IBAN, con un botón para copiarlos con un toque. Es usted quien realiza la transferencia, una vez al año, cuando le convenga.</li><li><strong>En efectivo</strong> — simplemente indíquenos su dirección y una fecha que le venga bien, en un mensaje ya preparado por WhatsApp, Viber o email. Le enviamos entonces a nuestro comercial directamente a su local para recoger el pago en persona, sin que tenga que desplazarse <em>(en Grecia, este servicio está disponible actualmente en Atenas)</em>.</li></ul><p>¿Quiere continuar con nosotros? Basta con abonar su vencimiento como se indica arriba. ¿Prefiere hacer una pausa? No haga nada: su menú simplemente se pone en reposo, sus datos permanecen intactos entre bastidores, y puede volver cuando quiera, con un clic. <strong>0 compromiso — siempre decide usted.</strong></p>'
  },
  tip:{
    fr:"Vous restez toujours maître de votre paiement : jamais de prélèvement, jamais de surprise — c'est vous qui choisissez le moment et le moyen qui vous conviennent le mieux.",
    en:"You always stay in control of your payment: never a debit, never a surprise — you choose the timing and the method that suits you best.",
    el:'Παραμένετε πάντα κύριος της πληρωμής σας: καμία χρέωση, καμία έκπληξη — εσείς επιλέγετε τη στιγμή και τον τρόπο που σας ταιριάζουν καλύτερα.',
    de:'Sie behalten immer die volle Kontrolle über Ihre Zahlung: nie eine Abbuchung, nie eine Überraschung — Sie wählen den Zeitpunkt und die Methode, die am besten zu Ihnen passt.',
    es:'Usted siempre mantiene el control de su pago: nunca un cargo, nunca una sorpresa — usted elige el momento y el método que mejor le convenga.'
  },
  kw:{fr:['paiement','virement','iban','espèces','commercial','prélèvement'],en:['payment','bank transfer','iban','cash','representative','automatic debit'],el:['πληρωμή','έμβασμα','μετρητά','χρέωση'],de:['zahlung','überweisung','bar','abbuchung'],es:['pago','transferencia','efectivo','comercial','cargo']}
},

{
  id:'adm-essai-forfait', cat:'admin', img:'09-admin-abonnement.webp',
  title:{
    fr:"Période d'essai gratuite et changement de forfait",
    en:'Free trial period and changing your plan',
    el:'Δωρεάν δοκιμαστική περίοδος και αλλαγή πακέτου',
    de:'Kostenlose Testphase und Paketwechsel',
    es:'Período de prueba gratuito y cambio de plan'
  },
  imgAlt:{fr:'Bandeau de période d\'essai',en:'Trial period banner',el:'Ταινία δοκιμαστικής περιόδου',de:'Testphasen-Banner',es:'Aviso de período de prueba'},
  body:{
    fr:"<p>À la création de votre compte, vous démarrez avec une <strong>période d'essai gratuite</strong> : aucun frais, vous explorez toutes les fonctionnalités librement. Votre abonnement démarre automatiquement à la fin de cette période — la date exacte est indiquée en haut de la section <strong>Mon abonnement</strong>.</p><p>Vous pouvez changer de forfait à tout moment depuis cette même section : passer de <strong>Menu QR</strong> à <strong>Commandes & Services</strong> (ou l'inverse), toujours en facturation annuelle. Pendant l'essai ou dans les jours avant une échéance, le changement est immédiat. En cours d'abonnement déjà payé, un passage à un forfait inférieur est simplement programmé pour la fin de votre période en cours — vous gardez vos fonctionnalités actuelles jusque-là — tandis qu'un passage à un forfait supérieur vous montre d'abord le montant exact à régler, calculé au prorata du temps qu'il vous reste, à valider avant tout paiement.</p>",
    en:'<p>When your account is created, you start with a <strong>free trial period</strong>: no fees, you explore every feature freely. Your subscription starts automatically at the end of this period — the exact date is shown at the top of the <strong>My subscription</strong> section.</p><p>You can change your plan at any time from this same section: switch from <strong>Menu QR</strong> to <strong>Orders & Services</strong> (or back), always billed yearly. During the trial or in the days before a due date, the change is immediate. Once a paid subscription is already running, switching to a lower plan is simply scheduled for the end of your current period — you keep your current features until then — while switching to a higher plan first shows you the exact amount due, calculated in proportion to your remaining time, to confirm before any payment.</p>',
    el:'<p>Κατά τη δημιουργία του λογαριασμού σας, ξεκινάτε με μια <strong>δωρεάν δοκιμαστική περίοδο</strong>: χωρίς χρεώσεις, εξερευνάτε ελεύθερα όλες τις λειτουργίες. Η συνδρομή σας ξεκινά αυτόματα στο τέλος αυτής της περιόδου — η ακριβής ημερομηνία εμφανίζεται στο πάνω μέρος της ενότητας <strong>Η συνδρομή μου</strong>.</p><p>Μπορείτε να αλλάξετε πακέτο ανά πάσα στιγμή από την ίδια ενότητα: από <strong>Menu QR</strong> σε <strong>Παραγγελίες & Υπηρεσίες</strong> (ή αντίστροφα), πάντα με ετήσια χρέωση. Κατά τη δοκιμαστική περίοδο ή τις ημέρες πριν από μια λήξη, η αλλαγή είναι άμεση. Σε ήδη πληρωμένη συνδρομή, η μετάβαση σε χαμηλότερο πακέτο προγραμματίζεται απλώς για το τέλος της τρέχουσας περιόδου σας — κρατάτε τις τρέχουσες λειτουργίες σας μέχρι τότε — ενώ η μετάβαση σε ανώτερο πακέτο σας δείχνει πρώτα το ακριβές ποσό που οφείλεται, υπολογισμένο αναλογικά με τον χρόνο που σας απομένει, για επιβεβαίωση πριν από κάθε πληρωμή.</p>',
    de:'<p>Bei der Erstellung Ihres Kontos starten Sie mit einer <strong>kostenlosen Testphase</strong>: keine Gebühren, Sie erkunden alle Funktionen frei. Ihr Abonnement startet automatisch am Ende dieser Phase — das genaue Datum wird oben im Bereich <strong>Mein Abonnement</strong> angezeigt.</p><p>Sie können Ihr Paket jederzeit aus demselben Bereich ändern: von <strong>Menu QR</strong> zu <strong>Bestellungen & Service</strong> wechseln (oder zurück), stets jährlich abgerechnet. Während der Testphase oder in den Tagen vor einem Fälligkeitsdatum erfolgt der Wechsel sofort. Bei einem bereits laufenden bezahlten Abonnement wird ein Wechsel zu einem kleineren Paket einfach für das Ende Ihres aktuellen Zeitraums geplant — Sie behalten Ihre aktuellen Funktionen bis dahin — während ein Wechsel zu einem größeren Paket Ihnen zunächst den genauen fälligen Betrag zeigt, anteilig zu Ihrer verbleibenden Zeit berechnet, zur Bestätigung vor jeder Zahlung.</p>',
    es:'<p>Al crear su cuenta, comienza con un <strong>período de prueba gratuito</strong>: sin cargos, explora todas las funciones libremente. Su suscripción comienza automáticamente al final de este período — la fecha exacta se muestra en la parte superior de la sección <strong>Mi suscripción</strong>.</p><p>Puede cambiar de plan en cualquier momento desde esa misma sección: pasar de <strong>Menu QR</strong> a <strong>Pedidos y Servicios</strong> (o viceversa), siempre con facturación anual. Durante la prueba o en los días previos a un vencimiento, el cambio es inmediato. Con una suscripción ya pagada en curso, pasar a un plan inferior simplemente se programa para el final de su período actual — mantiene sus funciones actuales hasta entonces — mientras que pasar a un plan superior le muestra primero el importe exacto a pagar, calculado en proporción al tiempo que le queda, para confirmar antes de cualquier pago.</p>'
  },
  tip:{
    fr:"Aucune surprise : le montant exact à payer vous est toujours montré et à confirmer, avant qu'aucun paiement ne soit jamais demandé.",
    en:"No surprises: the exact amount due is always shown to you to confirm, before any payment is ever requested.",
    el:'Καμία έκπληξη: το ακριβές οφειλόμενο ποσό σάς εμφανίζεται πάντα για επιβεβαίωση, πριν ζητηθεί ποτέ οποιαδήποτε πληρωμή.',
    de:'Keine Überraschungen: Der genaue fällige Betrag wird Ihnen immer zur Bestätigung angezeigt, bevor jemals eine Zahlung verlangt wird.',
    es:'Sin sorpresas: el importe exacto a pagar siempre se le muestra para confirmar, antes de que se le pida ningún pago.'
  },
  kw:{fr:['essai','gratuit','forfait','upgrade','downgrade','changement'],en:['trial','free','plan','upgrade','downgrade','change'],el:['δοκιμή','δωρεάν','πακέτο','αναβάθμιση'],de:['testphase','kostenlos','paket','upgrade'],es:['prueba','gratis','plan','mejora','cambio']}
},
...
/* ───────────── SERVEUR ───────────── */
{
  ...
  body:{
    fr:"<p>L'application <strong>GeNext Staff</strong> s'installe simplement en la cherchant sur le <strong>Google Play Store</strong>, comme n'importe quelle application Android, sur le téléphone de chaque membre de votre équipe.</p><p>Au premier lancement, saisissez l'identifiant de votre établissement — il reste enregistré ensuite, votre équipe n'a plus qu'à ouvrir l'application. L'indicateur <strong>Connecté</strong> en haut de l'écran confirme que tout fonctionne.</p>",
    en:"<p>The <strong>GeNext Staff</strong> app installs simply by searching for it on the <strong>Google Play Store</strong>, like any Android app, on each team member's phone.</p><p>On first launch, enter your venue's ID — it stays saved afterwards, your team just has to open the app. The <strong>Connected</strong> indicator at the top of the screen confirms everything works.</p>",
    el:'<p>Η εφαρμογή <strong>GeNext Staff</strong> εγκαθίσταται απλά αναζητώντας την στο <strong>Google Play Store</strong>, όπως κάθε εφαρμογή Android, στο τηλέφωνο κάθε μέλους της ομάδας σας.</p><p>Κατά την πρώτη εκκίνηση, εισαγάγετε το αναγνωριστικό του καταστήματός σας — παραμένει αποθηκευμένο στη συνέχεια, η ομάδα σας απλώς ανοίγει την εφαρμογή. Η ένδειξη <strong>Connecté</strong> στο πάνω μέρος της οθόνης επιβεβαιώνει ότι όλα λειτουργούν.</p>',
    de:'<p>Die App <strong>GeNext Staff</strong> installieren Sie einfach über die Suche im <strong>Google Play Store</strong>, wie jede andere Android-App, auf dem Handy jedes Teammitglieds.</p><p>Beim ersten Start geben Sie die Kennung Ihres Betriebs ein — sie bleibt danach gespeichert, Ihr Team muss die App nur noch öffnen. Die Anzeige <strong>Connecté</strong> oben im Bildschirm bestätigt, dass alles funktioniert.</p>',
    es:'<p>La aplicación <strong>GeNext Staff</strong> se instala simplemente buscándola en <strong>Google Play Store</strong>, como cualquier aplicación Android, en el teléfono de cada miembro de su equipo.</p><p>En el primer inicio, introduzca el identificador de su local — queda guardado después, su equipo solo tiene que abrir la aplicación. El indicador <strong>Connecté</strong> en la parte superior confirma que todo funciona.</p>'
  },
  tip:{
...

{
  ...
  body:{
    fr:"<p>Avec la formule <strong>Commandes & Services</strong>, chaque plat ajouté au panier peut être envoyé directement en cuisine — le client choisit <strong>🪑 À table</strong> (le numéro de sa table est déjà connu grâce au QR code) ou <strong>🥡 À emporter</strong>, valide sa commande, et votre équipe la reçoit en quelques secondes sur l'application Serveur.</p><p>Un bouton d'appel dédié permet aussi de solliciter un serveur sans passer commande (besoin d'aide, addition, etc.), toujours avec le numéro de table transmis automatiquement.</p>",
    en:"<p>With the <strong>Orders & Services</strong> plan, every dish added to the cart can be sent straight to the kitchen — the customer picks <strong>🪑 Dine in</strong> (their table number is already known from the QR code) or <strong>🥡 Takeaway</strong>, confirms their order, and your team receives it within seconds on the Waiter App.</p><p>A dedicated call button also lets customers request a waiter without ordering (need help, the bill, etc.), always with the table number sent automatically.</p>",
    el:'<p>Με το πακέτο <strong>Παραγγελίες & Υπηρεσίες</strong>, κάθε πιάτο που προστίθεται στο καλάθι μπορεί να σταλεί απευθείας στην κουζίνα — ο πελάτης επιλέγει <strong>🪑 Για τραπέζι</strong> (ο αριθμός του τραπεζιού του είναι ήδη γνωστός από τον κωδικό QR) ή <strong>🥡 Για take away</strong>, επιβεβαιώνει την παραγγελία του, και η ομάδα σας τη λαμβάνει μέσα σε δευτερόλεπτα στην Εφαρμογή Σερβιτόρου.</p><p>Ένα ειδικό κουμπί κλήσης επιτρέπει επίσης να ζητηθεί σερβιτόρος χωρίς παραγγελία (βοήθεια, λογαριασμός κ.λπ.), πάντα με τον αριθμό τραπεζιού να μεταδίδεται αυτόματα.</p>',
    de:'<p>Mit dem Paket <strong>Bestellungen & Service</strong> kann jedes zum Warenkorb hinzugefügte Gericht direkt an die Küche gesendet werden — der Gast wählt <strong>🪑 Am Tisch</strong> (die Tischnummer ist dank des QR-Codes bereits bekannt) oder <strong>🥡 Zum Mitnehmen</strong>, bestätigt seine Bestellung, und Ihr Team erhält sie innerhalb von Sekunden auf der Kellner-App.</p><p>Eine eigene Ruf-Schaltfläche ermöglicht es Gästen auch, einen Kellner ohne Bestellung zu rufen (Hilfe, Rechnung usw.), stets mit automatisch übermittelter Tischnummer.</p>',
    es:'<p>Con el plan <strong>Pedidos y Servicios</strong>, cada plato añadido al carrito puede enviarse directamente a la cocina — el cliente elige <strong>🪑 En la mesa</strong> (su número de mesa ya se conoce gracias al código QR) o <strong>🥡 Para llevar</strong>, confirma su pedido, y su equipo lo recibe en segundos en la aplicación de camarero.</p><p>Un botón de llamada específico también permite solicitar a un camarero sin pedir nada (ayuda, la cuenta, etc.), siempre con el número de mesa transmitido automáticamente.</p>'
  },
  kw:{fr:['commander','appeler','serveur','panier','à emporter','à table'],en:['order','call','waiter','cart','takeaway','dine in'],el:['παραγγελία','κλήση','take away'],de:['bestellen','rufen','mitnehmen'],es:['pedir','llamar','para llevar']}
},
